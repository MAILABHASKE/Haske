...
export default {
    ...
    data() {
        return {
            ...
            isWsiSeries: false,
            hasReportButton: true, // Control visibility of the report button
            isReportButtonEnabled: true, // Control whether the button is enabled
            modalitiesList: [],
           // reportState:'filled',
           senderName: '',
           doctorEmail: '',
           isMRIModality: false,
           patientID: '',
           isMRIQCButtonLoading: false
          
        };
    },
    watch: {

        async resourceOrthancId(newId) {
            if (this.isStudyLevel && newId) {
                await this.checkMRIModality();
                await this.extractPatientID();
            }
        },
    ...
    async mounted() {
        if (this.isStudyLevel && this.resourceOrthancId) {
            await this.checkMRIModality();
            await this.extractPatientID();
        }
    },
    methods: {
        ...
        async goToAIAnalysis(orthancId) {
  try {
    ...
    if (response.data.status === 'queued') {
      this.redirectToAI(response.data.jobId);
    } else {
      alert(response.data.message || 'AI analysis started');
    }
  } catch (error) {
...
redirectToAI(jobId) {
  const reactAppUrl = `https://www.haske.online/haske-ai?jobId=${jobId}`;
  window.open(reactAppUrl, '_blank');
},


  async checkMRIModality() {
            try {
                this.isMRIModality = false; // Reset before checking
                const seriesList = await api.getStudySeries(this.resourceOrthancId);
                
                if (seriesList && seriesList.length > 0) {
                    for (const series of seriesList) {
                        if (series.MainDicomTags && series.MainDicomTags.Modality === 'MR') {
                            this.isMRIModality = true;
                            break;
                        }
                    }
                }
            } catch (error) {
                console.error('Error checking modality:', error);
                this.isMRIModality = false;
            }
        },

        async extractPatientID() {
            try {
                const studyData = await api.getStudy(this.resourceOrthancId);
                if (studyData.PatientMainDicomTags && studyData.PatientMainDicomTags.PatientID) {
                    this.patientID = studyData.PatientMainDicomTags.PatientID;
                } else {
                    // Fallback to study ID if PatientID not available
                    this.patientID = this.resourceOrthancId.substring(0, 8);
                }
            } catch (error) {
                console.error('Error extracting PatientID:', error);
                this.patientID = this.resourceOrthancId.substring(0, 8);
            }
        },
     
async sendToMRIQC() {
    this.isMRIQCButtonLoading = true;
    this.toast.add({
        severity: 'info',
        summary: 'Processing',
        detail: 'Preparing study for MRIQC...',
        life: 3000
    });
    try {
                // Use the configured apiClient instead of axios directly
                const response = await apiClient.post('/mriqc/process', {
                    orthancId: this.resourceOrthancId,
                    patientID: this.patientID
                });
        
        if (response.data?.success) {
            const streamlitUrl = new URL('https://haske-mriqc.streamlit.app/');
            streamlitUrl.searchParams.set('patient_id', response.data.patientID);
            streamlitUrl.searchParams.set('orthanc_id', this.resourceOrthancId);
            
            // If direct download available, pass the archive URL
            if (response.data.directDownload) {
                streamlitUrl.searchParams.set('archive_url', encodeURIComponent(response.data.archiveUrl));
            }
            
            window.open(streamlitUrl.toString(), '_blank');
            
            this.toast.add({
                severity: 'success',
                summary: 'Success',
                detail: 'Opening MRIQC processor...',
                life: 5000
            });
        } else {
        ...
        let errorMessage = 'Failed to prepare study';
        if (error.response) {
        ...
        this.isMRIQCButtonLoading = false;
    }
},

         async goToReportPage(resourceOrthancId) {
        if (!resourceOrthancId && this.resourceOrthancId) {
            resourceOrthancId = this.resourceOrthancId;
        }
        
        

        if (resourceOrthancId) {
            // Navigate to the report page and pass the Orthanc ID as a route parameter
           const routeData =  this.$router.resolve({ 
                name: 'reportPage', 
                params: { 
                    id: resourceOrthancId,
       ...
       window.open(routeData.href, '_blank');
            ...
            console.warn('No resourceOrthancId provided!');
        }
    },

  async send(sendType) {
    try {
      if (!this.senderName || !this.doctorEmail) {
        alert("Please fill all fields!");
        return;
      }

      await this.$http.post('https://haske.online:8090/send-report', {
        orthancId: this.resourceOrthancId,
          studyInstanceUID: this.studyMainDicomTags.StudyInstanceUID,
          sendType,
...
reportButtonClass() {
  const status = localStorage.getItem(`reportStatus_${this.resourceOrthancId}`) || 'empty';

  switch (status) {
    case 'empty':
      return 'btn-secondary'; // Red for empty
    case 'filling':
      return 'btn-danger'; // Grey for findings only
    case 'filled':
      return 'btn-success'; // Green for findings + signature
    default:
      return 'btn-light';
  }
},
...
<template>
    <div>
        ...
        <div v-if="hasReportButton" class="btn-group">
            <button
                :class="['btn btn-sm m-1', reportButtonClass]"
                type="button"
                @click="goToReportPage(resourceOrthancId)"
            >
                <i class="bi bi-file-earmark-text" data-bs-toggle="tooltip" :title="$t('Report')"></i>
            </button>
            </div>

            <div class="btn-group">
                        <button 
            @click="goToAIAnalysis(resourceOrthancId)"
            class="btn btn-sm m-1 neutral"
            :disabled="!resourceOrthancId"
            >
            <i class="bi bi-robot" data-bs-toggle="tooltip" title="Haske MedAI"></i>
           
            </button>

            </div>
            <div class="btn-group">
            <button v-if="isStudyLevel && isMRIModality" 
            @click="sendToMRIQC"
            :disabled="isMRIQCButtonLoading"
            class="btn btn-sm m-1 neutral">
        <i v-if="isMRIQCButtonLoading" class="fas fa-spinner fa-spin"></i>
        <i v-else class="fas fa-brain" title="Haske MRI Quality Check"></i>
    </button>
</div>
        ...
        <div class="btn-group">
            ...
            <ul class="dropdown-menu p-3" style="width: 300px;">
                <li>
                <h6 class="dropdown-header">Send Options</h6>
                </li>
                <li>
                <div class="mb-3">
                    <label class="form-label">Your Name</label>
                    <input v-model="senderName" class="form-control form-control-sm">
                </div>
                </li>
                <li>
                <div class="mb-3">
                    <label class="form-label">Doctor's Email</label>
                    <input v-model="doctorEmail" type="email" class="form-control form-control-sm">
                </div>
                </li>
                <li>
                <div class="d-flex justify-content-between">
                   <button @click="send('both')" class="btn btn-sm" style="background-color: rgb(221, 132, 26); color: white;">
                    Send Both
                    </button>
                    <button @click="send('image')" class="btn btn-sm" style="background-color: rgb(221, 132, 26); color: white;">
                    Images Only
                    </button>
                    <button @click="send('report')" class="btn btn-sm" style="background-color: rgb(221, 132, 26); color: white;">
                    Report Only
                    </button>
